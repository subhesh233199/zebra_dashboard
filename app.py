...
{{
    "metrics": {{
        "Open ALL RRR Defects": {{"ATLS": [{{"version": "25.1", "value": N, "status": "TEXT"}}, ...], "BTLS": [...]}},
...
{{
    "metrics": {{
        "Open ALL RRR Defects": {{
            "ATLS": [
                {{"version": "25.1", "value": 10, "status": "RISK"}},
                {{"version": "25.2", "value": 8, "status": "MEDIUM RISK"}},
                {{"version": "25.3", "value": 5, "status": "ON TRACK"}}
            ],
            "BTLS": [...]
        }},
        "Customer Specific Testing (UAT)": {{
...
12. Use the following metric lists for iteration:
    ...
    overview_task = Task(
        description="""Write ONLY the following Markdown section:
## Overview
- Provide a 3-4 sentence comprehensive summary of release health, covering overall stability, notable improvements, and any concerning patterns observed across releases 25.1, 25.2, and 25.3
...
Only output this section.""",
    ...
    metrics_summary_task = Task(
        description="""Write ONLY the '## Metrics Summary' section with the following order:
...
async def run_full_analysis(request: FolderPathRequest) -> AnalysisResponse:
    ...
    # Get sub-crews
    data_crew, report_crew, viz_crew = setup_crew(full_source_text, llm)
